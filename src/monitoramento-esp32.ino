...
#include <MHZ19.h>
#include <Preferences.h> // <<< ADIÇÃO: Para salvar calibração
...
Preferences preferences; // <<< ADIÇÃO: Objeto para salvar dados na NVS
...
void calibrateMQ135() {
    ...
    // <<< MODIFICAÇÃO: Salva o valor R0 na NVS
...
void setupSensorMQ135() {
    ...
    // <<< MODIFICAÇÃO: LÓGICA DE CARREGAMENTO >>>
...
void setup(void) {
    ...
    // <<< MODIFICAÇÃO: Verifica se o botão de forçar calibração está pressionado
...
void loop(void) {
    ...
    if (now - lastMsg > interval) {
        ...
        MQ135.setA(-462.43); MQ135.setB(-2.204);
        lastNO2 = MQ135.readSensor(false, 0);

        MQ135.setA(47.209); MQ135.setB(-1.186);
        lastCL2 = MQ135.readSensor(false, 0);

        MQ135.setA(23.943); MQ135.setB(-1.11);
